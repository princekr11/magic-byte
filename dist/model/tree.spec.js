...
describe("tree", () => {
    it("Creates complex node", () => {
        const tree = (0, tree_1.createComplexNode)("mpe", ["0x00", "0x01"]);
        expect(tree.bytes["0x00"].bytes["0x01"]).toHaveProperty("matches");
        expect(tree.bytes["0x00"].bytes["0x01"]["matches"][0].typename).toBe("mpe");
    });
    it("Merges trees", () => {
        const tree = (0, tree_1.createComplexNode)("pic", ["0x00"]);
        const dba = (0, tree_1.createNode)("dba", ["0x00", "0x01", "0x02", "0x03"]);
        const merged = (0, tree_1.merge)(dba, tree);
    ...
    it("Merges overlapping", () => {
        const tree = (0, tree_1.createComplexNode)("pic", ["0x00"]);
        const dba = (0, tree_1.createNode)("pif", ["0x00"]);
        const merged = (0, tree_1.merge)(dba, tree);
        expect(merged.bytes["0x00"].matches).toHaveLength(2);
    });
    it("Merges deep overlapping", () => {
        const gifA = (0, tree_1.createComplexNode)("gif", ["0x47", "0x49", "0x46", "0x38", "0x37", "0x61"], { mime: "image/gif", extension: "gif" });
        const gifB = (0, tree_1.createNode)("gif", ["0x47", "0x49", "0x46", "0x38", "0x38", "0x61"], { mime: "image/gif", extension: "gif" });
        const gifC = (0, tree_1.createNode)("gif", ["0x47", "0x49", "0x46", "0x38", "0x39", "0x61"], { mime: "image/gif", extension: "gif" });
        const mergeA = (0, tree_1.merge)(gifB, gifA);
        const mergeB = (0, tree_1.merge)(gifC, mergeA);
