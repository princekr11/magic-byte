...
const add = (typename, signature, additionalInfo, offset) => {
    ...
    if (offset) {
        const existing = tree.offset[(0, toHex_1.toHex)(offset)];
        if (!existing) {
            tree.offset[(0, toHex_1.toHex)(offset)] = (0, tree_1.createComplexNode)(typename, signature.map((e) => e.toLowerCase()), additionalInfo);
        }
        else {
            const merged = (0, tree_1.merge)((0, tree_1.createNode)(typename, signature.map((e) => e.toLowerCase()), additionalInfo), { ...existing });
            tree.offset[(0, toHex_1.toHex)(offset)] = merged;
        }
    }
    else {
        if (tree.noOffset === null) {
            tree.noOffset = (0, tree_1.createComplexNode)(typename, signature.map((e) => e.toLowerCase()), additionalInfo);
        }
        else {
            tree.noOffset = (0, tree_1.merge)((0, tree_1.createNode)(typename, signature.map((e) => e.toLowerCase()), additionalInfo), tree.noOffset);
        }
    }
};
add("dbf", ["0x83"], {
