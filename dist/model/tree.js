...
const merge = (node, tree) => {
    ...
    if (tree.bytes[currentByte]) {
        tree.bytes[currentByte] = (0, exports.merge)((0, exports.createNode)(node.typename, path, node.info), tree.bytes[currentByte]);
        return tree;
    }
    ...
    if (!tree.bytes[currentByte]) {
        tree.bytes[currentByte] = {
            ...tree.bytes[currentByte],
            ...(0, exports.createComplexNode)(node.typename, path, node.info),
        };
    }
    return tree;
};
...
const createComplexNode = (typename, bytes, info) => {
    ...
    obj.bytes[currentKey] = (0, exports.createComplexNode)(typename, path, info);
    return obj;
};
